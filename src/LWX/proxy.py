...
class RequestParser:
    ...
    def consume(self, data: bytes):
        ...
        if not self.is_connected:
            ...
            if n < 12:
                return
            endianess, major, minor, name_len, data_len = struct.unpack(
            ...
            self.queued_bytes = self.queued_bytes[12:]

        n = len(self.queued_bytes)
        if n < 32:
            return
        opcode, reply_len = struct.unpack("BxH", self.queued_bytes[:4])

        print(f"Requested opcode: {opcode}")

        reply_bytes = 4 * reply_len
        if len(self.queued_bytes) < reply_bytes:
            return
        self.queued_bytes = self.queued_bytes[reply_bytes:]
...
class XServerToClientStream:
    def __init__(self, socket):
        ...
        self.byte_stream = EventReplyParser(socket)
...
class XClientToServerStream:
    def __init__(self, socket):
        ...
        self.parser = RequestParser(socket)
